...
public class NetworkExample2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 第二个虚拟机的优先级是 VM1 的两倍，因此将获得两倍的 CPU 时间
			vmid++;
			...
			// Datacenter0 将对应于 BRITE 节点 0
			int briteNode=0;
			NetworkTopology.mapNode(datacenter0.getId(),briteNode);

			// Datacenter1 将对应于 BRITE 节点 2
			briteNode=2;
			...
			Log.println("NetworkExample2 完成！");
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	private static Datacenter createDatacenter(String name){
		...
		// 在这个例子中，使用的 VMAllocatonPolicy 是带优先级的时间共享。
		// 这意味着虚拟机根据其优先级接收时间份额。
		hostList.add(
				new Host(
					hostId,
